...
# Old version of generate_tree_A
# This is bad because it assumes the norm_dict is the same for the classifier and regressor
# It should still work if the norm_dict is similar, but it's not a good practice
# def generate_tree(
#     classifier, regressor, root_halo_feat, t_out, n_max_iter=1000, norm_dict=None):
...
#                 torch.tensor(edge_index, dtype=torch.long),
#                 halo_curr_index)
#             x_feat_class = classifier.featurizer(halo_feats[path].unsqueeze(0)) # add batch dim
#             x_feat_regress = regressor.featurizer(halo_feats[path].unsqueeze(0)) # add batch dim
#             t_proj_class = classifier.time_proj_layer(t_next)
#             t_proj_regress = regressor.time_proj_layer(t_next)

#             # randomly sample the number of progenitors
#             x_classifier = torch.cat((x_feat_class, t_proj_class), dim=1)
#             yhat = classifier.classifier(x_classifier).softmax(dim=1)
#             num_progs = torch.multinomial(yhat, 1) + 1
#             num_progs = num_progs.item()
